...
**run_lua 只做内存里的计算 + 受限写回。** 以下均不可行：
- ❌ 访问网络 / fetch
- ❌ 持久化任何数据（localStorage / IndexedDB 不可写）
- ❌ 当作通用 shell（os.execute/io.popen 无宿主进程可调）
- ❌ 写 outputs 白名单之外的工作区路径（脚本对 MEMFS 的任意写跑完即毁，只有白名单同步回 VFS）

**可以**：
- ✅ 读 `files` 参数显式指定的工作区文件（经 MEMFS 只读副本注入，脚本 `io.open(path)` 读取）
- ✅ 读 `input`（stdin 回调 / MEMFS input.txt）
- ✅ **写回 `outputs` 白名单**：脚本 `io.open(path,'w')` 写 MEMFS → 求值后同步回工作区；
  **回传摘要（路径/大小/行数）而非全文**——长结果落盘，AI 需要时 read_file 按需取；
  `mutated: true` 走 undo 快照可撤销；Plan 模式带 outputs 被拦截

它等价于「在内存里跑一个计算器/转换器，配一份你递给它的只读文件副本，
和一个你批准它写回的出口白名单」，结果以摘要回传。
