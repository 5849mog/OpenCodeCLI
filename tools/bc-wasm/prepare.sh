#!/bin/bash
# prepare.sh — 获取 bc.wasm（本地测试用）
#
# 策略：
#   1. 如果 emcc 在 PATH 中 → 从源码编译（等同 CI）
#   2. 如果 public/wasm/bc.wasm 已存在 → 跳过（已就绪）
#   3. 否则 → 从预构建 URL 下载
#
# 用法：
#   ./prepare.sh              # 自动选择策略
...
TARGET="$OUT_DIR/bc.wasm"

# 如果已经存在且没要求强制重建
if [ -f "$TARGET" ] && [ "${FORCE_BUILD:-0}" != "1" ]; then
  echo "✓ bc.wasm 已就绪: $TARGET ($(du -h "$TARGET" | cut -f1))"
  exit 0
fi
...
echo "    浏览器打开后，bc 命令会自动回退到 JS 实现"
echo ""

# 尝试从 GitHub Release 下载（如果存在）
REPO_URL="${GITHUB_SERVER_URL:-https://github.com}/${GITHUB_REPOSITORY:-}"
if [ -n "$REPO_URL" ] && [ "$REPO_URL" != "https://github.com/" ]; then
  LATEST_RELEASE="$REPO_URL/releases/latest/download/bc.wasm"
...
# 都没有 → 报错提示
echo "✗ 无法获取 bc.wasm。请安装 emscripten 后运行 build.sh，"
echo "  或推送代码触发 CI 构建，然后 git pull 获取产物。"
echo ""
echo "  在此期间，bc 命令会使用 JS 降级实现（基本运算可用）。"
exit 1
