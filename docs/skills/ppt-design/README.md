...
| `scripts/inject_anim.py` | 714 | 动画：中文脚本 → OOXML timing 注入，PowerPoint COM 三层校验；菜单外效果走 `--raw` 手写 XML 逃生舱（须用户同意，逐条断言关闭＝降级） |
| `scripts/prep_assets.py` | 429 | 素材：无损调理（EXIF / sRGB / 裁切 / 300PPI）+ 扫描件清洗（白底 / 去斜 / 裁白边 / 对比） |
| `scripts/selftest.py` | 1021 | 回归自测：89 条断言，改任一脚本后必跑 |
...
改脚本后跑 `python scripts/selftest.py` 回归（89 条断言；node / pptxgenjs 不在默认 `NODE_PATH` 时会跳过若干链并注明）。
