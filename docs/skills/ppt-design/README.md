...
| `references/judge-prompt.md` | 83 | 闸门二视觉验收的 judge 填空模板（输出机器可读 JSON verdict） |
| `references/anim-atlas.md` | 116 | **动效手法库（取材用，不是规范）**：菜单内 8 条有权威依据的手法 + 菜单外 3 条（须 `--raw` 与用户同意）+ 依据登记表（哪个数字来自哪次探针） |
...
| `scripts/qa.py` | 1124 | 闸门一：几何 / 中文排版 / 对比度 / 交付合法性 / 表格外框裁剪 / 字体可移植性 / deck 级骨架 |
...
| `scripts/inject_anim.py` | 795 | 动画：中文脚本 → OOXML timing 注入，PowerPoint COM 三层校验；菜单外效果走 `--raw` 手写 XML 逃生舱（须用户同意，逐条断言关闭＝降级） |
| `scripts/prep_assets.py` | 429 | 素材：无损调理（EXIF / sRGB / 裁切 / 300PPI）+ 扫描件清洗（白底 / 去斜 / 裁白边 / 对比） |
| `scripts/selftest.py` | 1155 | 回归自测：102 条断言，改任一脚本后必跑 |
...
- **页内全自动连播，断点只在翻页。** 动画默认首条「自动」+ 其余「之后」，页内 0 点击——翻页由人控制、页内节奏由动画自己走完；有「点击」要写理由，qa 会告警。动效只有一个质量参数：**缓动**（默认缓入缓出，XML 编码抄自 PowerPoint 探针产物）。技能**不写自动换片时间**（`advTm`/`advClick`）、也不动放映设置，所以产物不会自己翻页；`qa.py` 会反查这三类风险。
...
改脚本后跑 `python scripts/selftest.py` 回归（102 条断言；node / pptxgenjs 不在默认 `NODE_PATH` 时会跳过若干链并注明）。
