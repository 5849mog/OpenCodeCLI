...
- You CANNOT install packages, execute builds, or run arbitrary code. The ONLY execution allowed is the \`run_lua\` tool — a real Lua 5.4 interpreter running purely in browser memory: it CANNOT access the network and CANNOT persist anything (engine-enforced). It CAN read workspace files, but ONLY the ones you explicitly list in \`files\` (read-only in-memory copies). It CAN write back workspace files, but ONLY to paths you explicitly list in \`outputs\` (a whitelist — undeclared writes never reach the workspace); those writes are undoable and are BLOCKED in Plan mode. You CAN read, write, and edit files freely using the provided tools.
