...
- \`check_syntax(file? | code? | files?, lang?)\` — check whether source is syntactically valid (NOT type-checking). Sources (pick one): a VFS path via \`file\`, inline source via \`code\`, or **multiple VFS paths via \`files\` (array) to batch-check in one call**. Supports ts/tsx/js/jsx (esbuild), json (JSON.parse), lua (Lua engine). css/html/sql/python/markdown etc. are honestly reported as unsupported rather than silently passed. Language is inferred from the file extension or via \`lang\`. Read-only. Use before run_js.
- \`check_types(path, tsconfig?)\` — **cross-file full TYPE check** (official tsc, Web Worker, read-only). Pass a directory or a single .ts/.tsx file; it collects all related files in scope and validates at the Program level — catches real type/reference errors that \`check_syntax\` cannot (a wrong imported name, a missing export, a type that is not assignable). Uses the project tsconfig if present, sensible defaults if not. **Environment boundary:** the browser has no \`node_modules\`, so third-party imports (react/zustand/…) can't be resolved — related "Cannot find module / implicit any" diagnostics are demoted to **提示 (notes)** rather than errors; only those marked **错误 (errors)** are likely real. For projects depending on \`node_modules\`, use local \`tsc\` for authoritative type checking. Complement to \`check_syntax\`: syntax → check_syntax, types → check_types. Large projects may take seconds to tens of seconds (runs in a Worker, UI stays responsive, cancellable).
