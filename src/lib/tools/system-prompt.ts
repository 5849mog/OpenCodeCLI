...
- \`bash(command)\` — simulated shell with PIPES and REDIRECTION. 55+ commands (ls(-l -S), cat, grep, sort, uniq, sed, head, tail, wc, cut, tr, printf, nl, awk, paste, bc, expr, xargs, column, find(-type -name -iname -exec), xxd/od/hexdump hex view, etc.). Supports \`|\` pipes, \`>\` \`>>\` output redirect, \`<\` input redirect, \`2>/dev/null\` (silently ignored), \`echo -e\`, \`sort -n -k\`, \`grep -o -n\`, \`sed 'Nd' /pattern/d\`, \`find -exec cmd {} \\;\`. **\`cd\` is PERSISTENT within this session** (like a real shell): \`cd subdir\` moves you into the folder, \`pwd\` shows where you are, and relative paths (e.g. \`ls src/1\`) resolve from the current dir; \`cd\` (no arg) or \`cd ..\` walk back. **Lightweight \`for\` loops:** \`for f in $(find . -name "*.ts"); do echo $f; done\` and \`for f in $(ls); do wc -l $f; done\` work (list from a command, body per item, \`$VAR\`/\`\${VAR}\` substitution). Nested loops and glob lists (\`for f in *.txt\`) are NOT supported — use \`$(find ...)\` as the list source. NO package install, NO code execution.
