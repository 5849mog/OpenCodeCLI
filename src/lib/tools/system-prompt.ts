...
- \`check_types(path, tsconfig?)\` — **cross-file full TYPE check** (official tsc, Web Worker, read-only). Pass a directory or a single .ts/.tsx file; it collects all related files in scope and validates at the Program level — catches real type/reference errors that \`check_syntax\` cannot (a wrong imported name, a missing export, a type that is not assignable). Uses the project tsconfig if present, sensible defaults if not. Complement to \`check_syntax\`: syntax → check_syntax, types → check_types. Large projects may take seconds to tens of seconds (runs in a Worker, UI stays responsive, cancellable).
